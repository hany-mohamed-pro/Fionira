...
- **(أ) ✅ RESOLVED — Stored-JE ↔ current-records sync (stale JEs).**
  ...
  - **B-أ-2 — OPEN (next step in Phase B):** *incremental* sync on `activate`/`replace` — a shared `syncJournalEntriesForFile(records, moduleType, fileHash, tenantId)` helper hooked at each `records.push` in `server.ts` (activate 2139, replace 2353, +3 minor), idempotent by `sourceFileId` (reusing the delete-by-file pattern at 3292). Needs its own decision/approval. Without it, the backfill must be re-run after new uploads.
