...
17. ✅ **خطأ V4 أُصلح قبل أي هجرة (2026-07-02)** — the Postgres-path field-casing bug (expert-audit-v2 finding V4): code read/emitted `Nontaxable_Amount` while the canonical record field is `NonTaxable_Amount` (`types.ts:21`, emitted by all processors). Free grep found it **wider than documented — 11 sites, two patterns**: (a) 5 read-sites `r.Nontaxable_Amount || r.nontaxable_amount` (db.ts:203 startup migration + server.ts activate/replace/restore/single-fix Postgres branches) that would have stored every exempt/non-taxable amount as **0** on first Postgres cutover; (b) 6 row→record mapping sites (`Nontaxable_Amount: Number(row.nontaxable_amount)`) that would have hidden the amount from every consumer (`TaxDeclaration`, `erp-engine:47`) in Postgres mode. All 11 normalized to `NonTaxable_Amount` (snake_case SQL column names untouched); esbuild-verified. Dev JSON mode was never affected (Postgres-only branches) — hence no live-data change. **Closed as a day-zero migration precondition.**
18. ✅ **ادعاء ZATCA أُزيل (2026-07-02)** — the UI claimed ZATCA-compliant smart invoices (`App.tsx:2167` «متوافقة مع متطلبات هيئة الزكاة» + the nav's implicit claim «فاتورة إلكترونية / إنشاء فواتير ضريبية» `NewAppShell.tsx:149`) while `SmartInvoice.tsx` contains **zero** QR/TLV/ZATCA logic (below even Phase-1, mandatory since Dec 2021 — expert-audit-v2 finding V1). Both texts replaced with honest function-only wording («إنشاء وإدارة الفواتير...») — no regulatory claim anywhere now. `TaxDeclaration.tsx`'s disclaimer kept as-is (legitimate external-authority direction, audited ✅ in `expert-system-principle-audit.md`). **الفوترة الإلكترونية المتوافقة (QR مرحلة-1 ثم Reporting/Clearance مرحلة-2) بند مستقبلي — المرحلة الثانية من خارطة الطريق** (expert-audit P16: sequential numbering + TLV-QR + server-side Gemini proxy + invoice→ledger integration).
...
- **(ب) ✅ RESOLVED (2026-07-02):** governance escalation now routes through the server — `POST /api/erp/governance/escalate-record` (`server.ts:~1621`) stores into the existing `devMemoryDb.governanceRequests` (persisted via `persistGovernanceRequests()`, `type: 'RECORD_ESCALATION'`), **idempotent** by (tenant, recordId, sessionId, PENDING). Client (`ValidationReviewScreen.tsx`): both real handlers (`handleSingleEscalate`, `handleFullEscalate`) call a shared `escalateRecordToServer` helper (dev-auth-safe token via `useAuth().user.getIdToken()`); the record flips to ESCALATED **only after** `res.ok && json.success` — never before, never inside a failing write. Full-escalate flips only the records whose save was confirmed. Live-proven: POST stored+persisted; duplicate POST returned `deduped:true` with count unchanged; distinct record added normally. Engines untouched. Design: `alerts-fixes-group1.md`.
  - **⚠️ Separate OPEN item (same defect class):** `override_audit_logs` — the exceptional-approval justification — is still written to Firestore **directly from the client** (`ValidationReviewScreen.tsx:~276`, inside `handleOverrideAndApprove`), so it is silently lost in dev exactly like escalation was. Fix with the same proven recipe (server endpoint + persisted store). Highest-value remaining governance hole (it concerns overrides specifically).
