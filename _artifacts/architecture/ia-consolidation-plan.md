...
5. ✅ **EXECUTED — Budget vs Actual** (TASK 5 + design e0fde690) — `BudgetVsActual.tsx` as a Reports tab after Income Statement + Owner Home one-line + hub card; budgets in `AppSettings.budgets`; Excel template + in-app editor; **Actual sourced exclusively from `computePnLCore` (zero drift, verified)**; sign-aware variance; branch-aware. Annual section-level MVP (monthly/per-category = next layers). Live-verified. See `ia-execution-phase3-budget-vs-actual.md`.
6. ⏳ **PENDING (Future, gated) — real Owners' Equity statement** — the ONLY remaining unimplemented roadmap item. Requires NEW data capture (owner contributions/drawings) + the chart-of-accounts-with-types foundation (shared with the real Balance Sheet / D12). Not part of the IA consolidation; a separate future milestone.
