...
export default function App() {
  ...
  const renderContentHeader = () => {
    ...
    if (type === 'GLOBAL_DASHBOARD') {
       ...
       else if (activeTab === 'balance_sheet') { pageTitle = isRTL ? 'الميزانية العمومية' : 'Balance Sheet'; subtitle = isRTL ? 'الميزانية الحقيقية من القيود المحاسبية الفعلية (متوازنة بلا تقدير) — مع النسخة التقديرية القديمة للمقارنة أدناه.' : 'Real balance sheet from actual journal entries (balanced, no estimate) — with the legacy estimated version below for comparison.'; }
