...
export function analyzeRevenues(records: FinancialRecord[], historicalData: FinancialRecord[] = []): IntelligenceResult[] {
    ...
    return records.map(record => {
        ...
        if (!record.Entity_Name && !record.Raw_Entity && !record.Entity_ID) {
            insights.push({ type: 'MISSING_CUSTOMER', severity: 'HIGH', message: 'عميل غير معروف في الإيرادات، قد يؤدي إلى مشاكل في الذمم', confidence: 1.0, scoreImpact: 35, suggestedAction: 'ربط الفاتورة بعميل مسجل' });
        }
        ...
        if (amount < 0) {
            ...
            if (!isCreditNote) {
                insights.push({ type: 'NEGATIVE_REVENUE', severity: 'HIGH', message: 'قيمة سالبة في الإيرادات وبلا سياق واضح كإشعار دائن', confidence: 0.9, scoreImpact: 20, suggestedAction: 'التحقق من طبيعة السجل' });
            } else {
                insights.push({ type: 'VALID_NEGATIVE_REVENUE', severity: 'LOW', message: 'مستند دائن / حركة عكسية معتمدة', confidence: 1.0, scoreImpact: 0 });
            }
        }
        ...
        if (taxContext === 'taxable' && vat === 0) {
            insights.push({ type: 'MISSING_EXPECTED_VAT', severity: 'HIGH', message: 'مبلغ مبيعات خاضع للضريبة ولكن تم تسجيل قيمة الضريبة بصفر', confidence: 0.95, scoreImpact: 35 });
            ...
            if (Math.abs(vat - expectedVat) > 1.0) {
                insights.push({ type: 'VAT_RATE_MISMATCH', severity: 'HIGH', message: 'إقرار ضريبي مشتبه به: قيمة ضريبة المبيعات لا تتطابق مع نسبة 15%', confidence: 0.9, scoreImpact: 30 });
            }
        }

        // 3. Duplication Rules
        const duplicateInvoice = context.historicalData.find(r => r.id !== record.id && r.Invoice_Number === record.Invoice_Number && record.Invoice_Number && (r.Entity_Name === record.Entity_Name || r.Raw_Entity === record.Raw_Entity)) || context.currentBatch.find(r => r.id !== record.id && r.Invoice_Number === record.Invoice_Number && record.Invoice_Number && (r.Entity_Name === record.Entity_Name || r.Raw_Entity === record.Raw_Entity));
        if (duplicateInvoice) {
             insights.push({ type: 'EXACT_DUPLICATE', severity: 'CRITICAL', message: `تكرار متطابق لرقم فاتورة المبيعات (${record.Invoice_Number}) لنفس العميل`, confidence: 1.0, scoreImpact: 50 });
        }
        ...
        if (record.Invoice_Date) {
            ...
            const now = new Date();
            if (date > now) {
                insights.push({ type: 'FUTURE_EXPENSE', severity: 'CRITICAL', message: 'تاريخ الإيراد في المستقبل (تلاعب أو خطأ إدخال)', confidence: 1.0, scoreImpact: 40 });
            } else {
                let fStart: Date;
                let fEnd: Date;
